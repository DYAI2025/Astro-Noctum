...
- Given the `profiles` table schema, then a row with `display_name IS NULL` cannot be inserted —
  the column is `NOT NULL DEFAULT ''`; the empty-string default exists solely to allow the
  signup trigger (`handle_new_user`) to create a profile row before the user completes onboarding;
  non-empty input is enforced by the form/backend validation, not by the database default
