...
- Given 50 existierende User in prod mit `soulprint_sectors = NULL`, when das Backfill-Script gegen prod ausgeführt wird, then erhalten alle 50 einen gültigen 12-Element-Array und die Query `SELECT COUNT(*) FROM astro_profiles WHERE soulprint_sectors IS NULL` ergibt 0.
