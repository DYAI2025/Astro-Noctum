...
| TASK-mini-signature-alignment-fix | MiniSignature visual alignment within Identity row — verify centering, responsive sizing, and canvas aspect ratio at all breakpoints | P1 | Done | - | - | 2026-04-06 | Removed aspect-square + justify-between (label overflowed tile); replaced with flex-col gap-3 + p-5; tile now correctly sizes to ring+label |
| TASK-current-sky-behavior-fix | Current Sky mode still shows insufficiently different output from birth sky — revalidate simTime calculation, observer coordinates, and constellation rendering | P1 | Done | - | TASK-current-sky-delta-fix | 2026-04-06 | simTime ✓, observer ✓. Birth constellation now highlighted gold in sky view; auto-aim camera at birth constellation on first planetarium entry; birthConstellationRef + hasCenteredOnBirthConRef added |
| TASK-daily-pulse-influences-live-fix | Daily Pulse and Influence Gauges do not show convincingly live/date-sensitive data — verify transit feed, date-boundary rotation, and gauge label semantics | P1 | Done | [REQ-F-dashboard-live-daily-signals](../1-objectives/requirements/REQ-F-dashboard-live-daily-signals.md) | - | 2026-04-06 | `planetInfluences.ts`: Kepler solver computes today's ecliptic longitude for Mars/Jupiter/Venus/Saturn → zodiac sector → aspect vs birth sign → fieldStrength + isResonant. InfluenceGauges now accepts `birthSign` prop; live computation when sign known; color shifts blue (resonant) ↔ red (tension); "%" replaced with ■□ field-strength blocks. Dashboard wires `apiData?.western?.zodiac_sign` as birthSign. 11 new tests green. |
