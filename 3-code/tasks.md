...
| ID | Task | Priority | Status | Req | Dependencies | Updated | Notes |
|----|------|----------|--------|-----|--------------|---------|-------|
| TASK-daily-verify-noaa | Make a test POST to FuFirE `/impact/active`; confirm `solar_pressure` field in response or decide pass-through approach; document result in ASM-noaa-in-fufre | P0 | Todo | [REQ-F-impact-active-endpoint](../1-objectives/requirements/REQ-F-impact-active-endpoint.md) | - | 2026-04-13 | Verify before coding harmony_index formula; see ASM-noaa-in-fufre |
| TASK-daily-impact-proxy | Add `POST /api/impact/active` proxy in server.mjs: requireUserAuth, 15-min cache keyed on (user_id, date), forward to FuFirE | P0 | Todo | [REQ-F-impact-active-endpoint](../1-objectives/requirements/REQ-F-impact-active-endpoint.md), [REQ-PERF-impact-active-response-time](../1-objectives/requirements/REQ-PERF-impact-active-response-time.md) | TASK-daily-verify-noaa | 2026-04-13 | Cache miss ≤800ms p95; cache hit ≤200ms |
