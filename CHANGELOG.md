...
- **SignatureReveal zeigte rohe i18n-Keys statt Übersetzungen** (`src/i18n/translations.ts` + `src/__tests__/signature-reveal-i18n.test.ts`) — fünf im Onboarding-Reveal genutzte Keys (`signatureReveal.soulprintCalculating`, `.signatureForming`, `.signaturePartialError`, `.previewNote`, `.continueAnyway`) hatten keine Entsprechung im EN- oder DE-Namespace, der Reveal zeigte daher rohe Schlüssel wie `signatureReveal.previewNote` statt Text. Bestehender `signature-reveal-cymatics.test.tsx` mockte `useLanguage` mit einer Fake-t-Map → Bug war für die Suite unsichtbar. Fix: Namespace `signatureReveal` mit 5 Keys zu beiden Sprachen ergänzt (EN + DE direkt nach `signatur:`). Regression-Guard: neuer `signature-reveal-i18n.test.ts` (10 Tests = 5 Keys × 2 Sprachen) nutzt den echten `translations`-Export, kein Mock.
