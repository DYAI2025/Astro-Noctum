...
Completed sprints, gap analyses, and approved goals are archived in [`docs/sprint-history.md`](docs/sprint-history.md) (newest first).

**Locked architectural decisions** (see `2-design/decisions/`):
- `bazodiac_engine/` → archived to `archive/bazodiac_engine/` (2026-05-01). Production source of truth is `packages/shared/src/signatur/bazodiac-engine.ts` + `server.mjs` + `src/lib/master-signal/`.
- Tagespuls endpoints live on `server.mjs` (`/api/daily-pulse`, `/api/daily-interpretation`), not Supabase Edge Functions. The OpenAPI spec under `apps/tagespuls_package/` is reference-only.
- Quiz data model is the hybrid in `DEC-quiz-data-model-migration`: `contribution_events` is the Ring source; `user_quiz_answers` (append-only, RLS) + `user_quiz_profile` (aggregated) feed the Kranz/agent layer.
...
├── archive/                # Archived reference impls (e.g. bazodiac_engine Python port)
...
| `src/components/BirthChartOrrery.tsx` | Three.js 3D solar system (Keplerian orbits). Shaders in `src/lib/3d/materials.ts`, orbital math in `src/lib/astronomy/` |
| `server.mjs` | Production Express server: BAFE proxy with fallback chain, Master Signal JS port, `/api/contribute`, `/api/space-weather/extended` (5-min cached NOAA+DONKI), Supabase admin auth, ElevenLabs tool endpoints, Stripe checkout + webhook |
| `src/lib/fusion-ring/` | Fusion Ring engine — signal computation, BaZi/Western/Wu-Xing layers, transit math, canvas draw utilities |
| `src/hooks/useFusionSignal.ts` | Polls `/api/transit-state/:userId`, parses via Zod `TransitStateSchema`, computes `FusionSignalData` |
...
| `src/components/quizzes/` | 22 quiz components; shared props `{ onComplete, onClose }`. Converters in `quiz-to-event.ts` |
| `src/components/fusion-ring-website/` | V2 Signatur engine (Cousto-frequency spirograph) — `bazodiac-engine.ts` (math), `FusionRingCanvasV2.tsx` (Three.js renderer), `signatur-bridge.ts` (12-sector → 7 planet/6 quiz adapters), `FusionRingWebsiteCanvas.tsx` (V1 fallback). Gated by `signature_engine_v2` |
...
Railway via `nixpacks.toml` + `railway.json`. Build: `npm ci && npm run build`. Start: `node server.mjs`. BAFE routing details (Fly.io primary + Railway legacy fallback) are in External Dependencies above.
