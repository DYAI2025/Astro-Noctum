...
**S-SOULPRINT-HOTFIX Progress (2026-04-19):** 4/6 Done. PR #281 auf Branch `fix/soulprint-persist-upsert`. Phase 1 komplett (upsert fix + code-analysis + 6 Vitest-Fälle). Phase 2 komplett — `scripts/backfill-soulprint.mjs` implementiert + Helper-Export `recomputeSoulprintFromAstroJson`; Dry-run gegen prod bestätigt 50/50 Kandidaten korrekt berechenbar (9 all-zero, 41 echte Sektoren). `--apply` pending user-Freigabe. Verbleibend: TASK-sphx-integration-test (Phase 3 post-deploy, 2 Test-User), TASK-sphx-po-report.
