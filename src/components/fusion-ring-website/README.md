...
| T4 | **Cousto audio mute** | `useCoustoAudio` (localStorage `cousto_audio_muted`) | User click | All 6 Cousto oscillators gain → 0. Mute state persists in localStorage. | < 100ms (per REQ AC) | **Verified** — `toggleMute()` → `ctx.suspend()` called synchronously within same React render cycle (<<1ms in practice). Mute state + volume persisted in localStorage and restored on reload. 14 tests covering init, toggle, persistence, reload sim, and timing contract. |
