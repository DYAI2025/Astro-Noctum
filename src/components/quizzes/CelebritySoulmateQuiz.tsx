...
import { celebritySoulmateToEvent } from '@/src/lib/signatur/quiz-to-event';
...
function ResultScreen({
  ...
  return (
    ...
    >
      ...
      {showShare && (
        <SharePopup
          quizTitle="Celebrity Soulmate"
          resultTitle={result.match.name}
    ...
    <div className="relative w-full h-full min-h-[500px] flex flex-col">
      <AnimatePresence mode="wait">
        ...
        {screen === 'result' && matchResult && (
          <ResultScreen key="result" result={matchResult} onClose={onClose} />
        )}
      </AnimatePresence>
    </div>
  );
